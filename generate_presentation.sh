...
extract_links() {
    if [[ ! -s "$DAT_IN_OUT" ]]; then
        ...
        normalise_svg | grep 'fill="none" class="id_.* id_' | sed 's/.* .*class="id_\([^ ]*\) id_\([^ ]*\)" d="M [^ ]* A \([^ ]*\) [0-9][0-9]*,[0-9][0-9]* 1,\([^ ]*\)" stroke="\([^"]*\)".*/\1 §\2§(\3~\4\5)/' | LC_ALL=c sort -u | collapse | LC_ALL=c sort > "$DAT_OUT"
        # TODO: Consider storing only the path-coordinates for one way
        normalise_svg | grep 'fill="none" class="id_.* id_' | sed 's/.* .*class="id_\([^ ]*\) id_\([^ ]*\)" d="M [^ ]* A \([^ ]*\) [0-9][0-9]*,[0-9][0-9]* 1,\([^ ]*\)" stroke="\([^"]*\)".*/\1 §\2§(\3~\4\5)/'| LC_ALL=c sort -u | collapse | LC_ALL=c sort > "$DAT_IN"
